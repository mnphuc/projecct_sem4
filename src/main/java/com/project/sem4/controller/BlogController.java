...
public class BlogController {
    ...
    @RequestMapping(value = "them-bai-viet", method = RequestMethod.GET)
...
//        Long userId = users.getUserID();
    ...
    public String updateBlog(Model model,HttpServletRequest request, @RequestParam(name = "id", required = false) Integer id, RedirectAttributes redirectAttributes){
        if (id == null ){
            String msg = "warning,Cảnh Báo,Bạn Đang Cố Tấn Công Hệ Thống Của Mình,hide";
            redirectAttributes.addAttribute("msg", msg);
            return "redirect:/quan-tri/blog/danh-sach-bai-viet";
        ...
        List<BlogCategories> blogCategories = blogRepository.getAllBlogCategory();
        Blogs blogs = blogRepository.findBlogById(id);
            model.addAttribute("blogs", blogs);
            model.addAttribute("blogCategories", blogCategories);
            return "admin/blog/editBlog";

    }
    @RequestMapping(value = "sua-bai-viet", method = RequestMethod.POST)
    public String saveUpdateBlog(@Valid Blogs blogs, BindingResult result, Model model, RedirectAttributes redirectAttributes){
        if (result.hasErrors()) {
            return "admin/blog/editBlog";
        }
        Boolean bl = blogRepository.editBlog(blogs);
        if (bl){
            String msg = "success,Thông Báo,Sửa Bài Viết Thành Công,hide";
            redirectAttributes.addFlashAttribute("msg", msg);
            return "redirect:/quan-tri/blog/danh-sach-bai-viet";
        }
        String msg = "error,Thất Bại,Có Lỗi Khi Sửa Bài Viết,hide";
        redirectAttributes.addFlashAttribute("msg", msg);
        return "admin/blog/editBlog";
    }
    @RequestMapping(value = "xoa-bai-viet", method = RequestMethod.POST)
    public String deleteBlog(@RequestParam (name = "id", required = false) Integer id, Model model, RedirectAttributes redirectAttributes){
        if (id == null ){
            String msg = "warning,Cảnh Báo,Bạn Đang Cố Tấn Công Hệ Thống Của Mình,hide";
            redirectAttributes.addFlashAttribute("msg", msg);
            return "redirect:/quan-tri/blog/danh-sach-bai-viet";
        }
        Boolean bl = blogRepository.deleteBlog(id);
        if (bl){
            String msg = "success,Thông Báo,Xóa Bài Viết Thành Công,hide";
            redirectAttributes.addFlashAttribute("msg", msg);
            return "redirect:/quan-tri/blog/danh-sach-bai-viet";
        }
        String msg = "error,Lỗi,Xóa Không Thành Công,hide";
